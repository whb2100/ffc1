...
$(function(){
    ...
    datagrid = $("#itemList").datagrid({
		...
		idField:'record_id',
		singleSelect:true,
		url:'data/top.retrieve.php?project_id='+projectId,
...
//		rownumbers:true,
		...
		columns:[[
		          {title:ACCOUNT_NUMBER,field:'main_sub',  width: '15%', align: 'left',halign: 'center'},
...
function searchNoMix(){
	...
	for (var i = 0; i < currencyData.length; i++) {
			...
			$("#currencyList"+currencyData[i].currency_type_id).datagrid({
				...
				url:'data/top.retrieve2.php?currency_type_id='+currencyData[i].currency_type_id,
				idField:'record_id',
...
function ExcleFunction(){
	...
	if (mix == 1) {
		window.open("data/report.excel.php?report_type=13&start_date=" + start_date, "_blank");
	} else {
		window.open("data/report.excel.php?report_type=3&start_date=" + start_date, "_blank");
	}
}
...
function PDFFunction(){
	...
	var start_date = formatEnDate($('#startDate').datebox('getValue'));
	if (mix == 1) {
		window.open("data/report.pdf.php?report_type=13&start_date=" + start_date, "_blank");
	} else {
		window.open("data/report.pdf.php?report_type=3&start_date=" + start_date, "_blank");
	}
}
/*获取高度*/
