...
$(function(){
    ...
    datagrid = $("#itemList").datagrid({
		...
		idField:'record_id',
		singleSelect:true,
		url:'data/balance.retrieve.php?project_id='+projectId,
...
function searchNoMix(){
	...
	for (var i = 0; i < currencyData.length; i++) {
			...
			$("#currencyList"+currencyData[i].currency_type_id).datagrid({
				...
				url:'data/balance.retrieve2.php?currency_type_id='+currencyData[i].currency_type_id,
				idField:'record_id',
...
function ExcleFunction(){
	...
	if (mix == 1) {
		window.open("data/report.excel.php?report_type=14&start_date=" + start_date, "_blank");
	} else {
		window.open("data/report.excel.php?report_type=5&start_date=" + start_date, "_blank");
	}
}
/*导出*/
function PDFFunction(){
	...
	var start_date = formatEnDate($('#startDate').datebox('getValue'));
	if (mix == 1) {
		window.open("data/report.pdf.php?report_type=14&start_date=" + start_date, "_blank");
	} else {
		window.open("data/report.pdf.php?report_type=5&start_date=" + start_date, "_blank");
	}
}
