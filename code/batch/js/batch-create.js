...
//	    total = parts.join(".");
//		if(total != 0){
//			$("#totalAmount").html("<span style='color:red'>"+total+"</span>");
//		}else{
...
function showBatch(code,id,currency_id,currency_code,currency_type){
	...
	$("#batch_title").html(LOCATION_BATCH+"<span style='font-size: 20px;'>"+currentProjectName+"></span>"+UPDATE_TITLE_BATCH);
...
function expandBatch(expandCode,expandId){
	...
	if(isExpand == 'none'){
		$("#list"+expandId).datagrid('options')['url'] = 'data/transaction.retrieve.php?batch_ids='+expandId;
...
function addBatch(){
	...
	$("#examSubjectDiv").hide();
	$("#examNumberDiv").hide();
	$("#batch_title").html(LOCATION_BATCH+"<span style='font-size: 20px;'>"+currentProjectName+"></span>"+ADD_TITLE_BATCH);
	$("#transactionCode").val("");
	$('#transactionDetailList').datagrid('loadData', { total: 0, rows: [] });
...
function examSubject(){
    ...
    if(appendCheckSub){
		...
		$("#subDiv").append('<div class="xmys_right_button" style="margin-right:-100px;"><ul><li class="xmys_button_download" style="margin-top: -10px;margin-right: 100px"><a  href="javascript:void(0)" onclick="ExcelSubFunction()"><em><img src="../images/icon_type.png"/></em>Excel</a></li></ul></div><br>');
