$(function(){
	$("#sysUserRegisterForm").form('clear');
	$('#departmentTree').tree({    
	...
	$('#departmentTree').tree({
   	...
   	// 验证两次输入的密码是否相同
		equalTo:{
			validator:function(value){
				var password = $("#formUserPassword").val();
				if(password != "" && value != ""){
	...
	//验证身份证号
       ...
       $("#allocatedRole").datagrid({
       	...
       	columns:[[
       	          {title:'权限ID',field:'role_id',width:100,align:'center',checkbox:true},
       	          {title:'权限名称',field:'role_name',width:150,align:'center'}
       	          ]]
       });	
       
     //部门选择
       $("#formDepartId").combobox({
       	url:'data/department.tree.retrieve.php',    
       	valueField:"dept_id",    
       	textField:"dept_name",
       	required:true
...
var IdCardValidateValideCode = [ 1, 0, 10, 9, 8, 7, 6, 5, 4, 3, 2 ];            // 身份证验证位值.10代表X   
function (idCard) { 
